...
class ShapeNetSDF(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Get list of randomly shuffled indices
        idcs = np.random.permutation(points_dict["random_points"].shape[0])
        idcs_random = idcs[:self.num_points[0]]
        idcs_surf = idcs[:self.num_points[1]]

        points = np.concatenate(
            [points_dict["random_points"][idcs_random], points_dict["surf_points"][idcs_surf]], axis=0, dtype=np.float32)

        sdf = np.concatenate(
            [points_dict["random_points_sdf"][idcs_random], points_dict["surf_points_sdf"][idcs_surf]],
            axis=0,
        )
        ...
        # Points are [-0.5 to 0.5], scale to [-1, 1]
        points = 2 * points
...
if __name__ == "__main__":
    train_dataset = ShapeNetSDF(
        root="/media/davidknigge/hard-disk1/storage/ShapeNetSDF", num_points=(1024, 1024)
    )

    points, sdf, label, idx = train_dataset[0]