...
class ShapeNet(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        idcs = np.random.permutation(random_points.shape[0])
        # idcs = np.random.permutation(10000)

        points = np.concatenate(
            [random_points[idcs[:self.num_points[0]]], random_points_near_surface[idcs[:self.num_points[1]]]], axis=0, dtype=np.float32
        )
        ...
        occupancies = np.concatenate(
            [random_points_occupancy[idcs[:self.num_points[0]]], random_points_near_surface_occupancy[idcs[:self.num_points[1]]]],
            axis=0,
            dtype=np.float32,
        )
        ...
        # Points are scaled to [-1, 1]
        points = 2 * points

        return points, occupancies, label, idx
